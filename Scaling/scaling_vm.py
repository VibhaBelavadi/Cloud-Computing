...
def scaling():
    ...
    for key in vm_cpuUtilization:
        ...
        if length_of_values >int(size)-1:
            ...
            if "CPU" in key:
                vm_cpuUtilization[key][0]>threshold
                flag_treshold = 1
	    if "MEM" in key:
		vm_cpuUtilization[key][0]>mem_treshold
                flag_treshold = 1
            if compare_time >= 120 and flag_treshold:
		if "CPU" in key:
                #CallScaling(key)
...
def check_for_scaling(key,status):
     ...
     if flag_stage:
         ...
         if status == "CPU":
	         ...
	         print "CPU Utilization of VM is high and perform Scaling"
             ...
             if host_memutilization  == 0:
                 ...
                 print "Memory Utilization is low and perform scaling "+str(used_memory)
                 if float(used_memory) > 0:
                     sc.scaling_memory(key,status,used_memory)
             else:
                 print "Memory Utilization is high migrate"
                 write_to_file(ip)
                 Migrate(str(ip),status)
         host_memutilization = 0
         host_cpuutilization = 0
